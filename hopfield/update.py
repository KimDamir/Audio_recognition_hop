...
def extended_storkey_update(sample, weights):
    ...
    numerics = 2*tf.compat.v1.cast(sample, weights.dtype) - 1
    row_sample = tf.compat.v1.expand_dims(numerics, axis=0)
    row_h = tf.compat.v1.matmul(row_sample, weights)
    ...
    return tf.compat.v1.assign_add(weights, scale * (pos_term - neg_term))
